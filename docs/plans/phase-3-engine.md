...
>
> **Open:** `Corpus.StartingKeys()` and the `startingKeys` constant are now two sources for one number, and both fake corpora implement the method as `return 0 // ignored`. Recommend deleting it from the interface - how many keys a beginner starts with is a pedagogy decision belonging beside `unlockKeyThreshold` in the tuning table, not a property of the language data - which also shrinks what `corpus.Provider` must satisfy in C2.
...
- **C2 - `corpus.Provider`.** `//go:embed data/corpus.json`, parse once at init, implement `KeyOrder` / `NgramsByFrequency` / `Transitions`. Swap it for the fake in the harness/integration tests.
