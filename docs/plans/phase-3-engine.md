...
- **Property-based (a seeded loop, _not_ `testing/quick`):** `internal/engine/property_test.go` - "any valid state ⇒ a lesson of only unlocked keys" is the canonical one the roadmap names, plus word shape and the `Targets` telemetry. Each runs 200 generated states built by `randomValidState`, whose one non-obvious job is keeping the unlocked set a **prefix** of `KeyOrder()` - the invariant `nextKeyToUnlock` depends on. **`testing/quick` was considered and rejected** _(Cory, 2026-08-20)_: the package is frozen ("not accepting new features", per its own doc), and reflection cannot build a valid `CompetencyState` - it would produce arbitrary runes and a non-prefix unlocked set, i.e. failures that say nothing about states the engine is ever handed. Supplying a `quick.Generator` to fix that is writing the generator by hand anyway, at which point a seeded loop is the same work and reproducible by construction. Go's native fuzzing is the modern successor but is aimed at byte-slice inputs, not structs with invariants. The roadmap's "e.g. `testing/quick`" named a technique, and the technique is what shipped.
